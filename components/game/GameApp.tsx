"use client";
/* Profile images are runtime Netlify Blob URLs, so the native img element is intentional. */
/* eslint-disable @next/next/no-img-element */
import {useCallback,useEffect,useMemo,useRef,useState} from "react";
import {audioManager,DEFAULT_AUDIO} from "../../lib/audio/audio-manager";
import {authApi,gameApi,type AccountUser} from "../../lib/game/client";
...
export function GameApp(){
  const [auth,setAuth]=useState<"AUTH_LOADING"|"UNAUTHENTICATED"|"AUTHENTICATED"|"ERROR">("AUTH_LOADING");
  const [user,setUser]=useState<AccountUser|null>(null);
  useEffect(()=>{void authApi.session().then(result=>{setUser(result.user);setAuth("AUTHENTICATED")}).catch(()=>setAuth("UNAUTHENTICATED"))},[]);
  if(auth==="AUTH_LOADING")return <main className="game"><ClubWorld/><LoadingState title="OPENING THE CLUB..." detail="Finding your table and restoring your profile."/></main>;
  if(auth==="ERROR")return <main className="game"><ClubWorld/><LoadingState title="THE DOOR IS STUCK" detail="Reload the page and try opening the club again."/></main>;
  if(auth==="UNAUTHENTICATED"||!user)return <Onboarding complete={next=>{setUser(next);setAuth("AUTHENTICATED")}}/>;
  return <ClubGame user={user}/>;
}

function Onboarding({complete}:{complete:(user:AccountUser)=>void}){
  const [step,setStep]=useState(0),[firstName,setFirstName]=useState(""),[lastName,setLastName]=useState(""),[nickname,setNickname]=useState("");
  const [avatar,setAvatar]=useState<File|null>(null),[preview,setPreview]=useState(""),[micState,setMicState]=useState<MicState>("idle"),[micLevel,setMicLevel]=useState(0),[error,setError]=useState(""),[busy,setBusy]=useState(false);
  const meterCleanup=useRef<undefined|(()=>void)>(undefined);
  useEffect(()=>()=>{meterCleanup.current?.()},[]);
  const selectPhoto=async(file?:File)=>{if(!file)return;if(!["image/jpeg","image/png","image/webp"].includes(file.type)||file.size>12_000_000){setError("Choose a JPG, PNG or WEBP photo under 12 MB.");return}const bitmap=await createImageBitmap(file);const size=Math.min(bitmap.width,bitmap.height),canvas=document.createElement("canvas");canvas.width=512;canvas.height=512;canvas.getContext("2d")?.drawImage(bitmap,(bitmap.width-size)/2,(bitmap.height-size)/2,size,size,0,0,512,512);canvas.toBlob(blob=>{if(!blob)return;const cropped=new File([blob],"avatar.webp",{type:"image/webp"});setAvatar(cropped);setPreview(URL.createObjectURL(cropped));setError("")},"image/webp",.86)};
  const setupMic=async()=>{setMicState("requesting");try{audioManager.unlock();const stream=await voiceManager.request();meterCleanup.current?.();meterCleanup.current=voiceManager.createMeter(stream,setMicLevel);setMicState(stream.getAudioTracks().some(track=>track.readyState==="live")?"ready":"denied")}catch{setMicState("denied")}};
  const create=async()=>{if(!avatar||micState!=="ready")return;setBusy(true);setError("");const form=new FormData();form.set("firstName",firstName);form.set("lastName",lastName);form.set("nickname",nickname);form.set("avatar",avatar);try{const result=await authApi.create(form);complete(result.user)}catch(e){setError(e instanceof Error?e.message:"Account creation failed.");setBusy(false)}};
  const validProfile=firstName.trim().length>=2&&lastName.trim().length>=2&&nickname.trim().length>=2;
  return <main className="game onboarding-shell" onPointerDownCapture={()=>audioManager.unlock()}><ClubWorld/><section className="onboarding-card"><div className="onboarding-progress">{[0,1,2,3].map(n=><i className={n<=step?"active":""} key={n}/>)}</div>
    {step===0&&<><p className="kicker">THE BASEMENT PRESENTS</p><h1>WELCOME TO<br/><em>THE CLUB</em></h1><p>Create your stage identity, check your microphone, then own the room.</p><GameButton primary onClick={()=>setStep(1)}>STEP INSIDE →</GameButton></>}
    {step===1&&<><p className="kicker">YOUR STAGE IDENTITY</p><h1>WHO’S ON<br/><em>THE BILL?</em></h1><div className="onboarding-fields"><label>FIRST NAME<input autoFocus maxLength={50} value={firstName} onChange={e=>setFirstName(e.target.value)}/></label><label>LAST NAME<input maxLength={50} value={lastName} onChange={e=>setLastName(e.target.value)}/></label><label className="wide">STAGE NAME<input maxLength={24} value={nickname} onChange={e=>setNickname(e.target.value.toUpperCase())}/><small>This is what other players see.</small></label></div><GameButton primary disabled={!validProfile} onClick={()=>setStep(2)}>CHOOSE YOUR PHOTO →</GameButton></>}
    {step===2&&<><p className="kicker">PROFILE PHOTO</p><h1>FACE OF<br/><em>THE ACT</em></h1><label className="avatar-upload"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>void selectPhoto(e.target.files?.[0])}/>{preview?<img src={preview} alt="Cropped profile preview"/>:<span>＋<small>UPLOAD PHOTO</small></span>}</label><p>Your photo is cropped to a square and compressed before upload.</p><GameButton primary disabled={!avatar} onClick={()=>setStep(3)}>SET UP YOUR MIC →</GameButton></>}
    {step===3&&<><p className="kicker">DEVICE CHECK</p><h1>SET UP<br/><em>YOUR MIC</em></h1><p>Comedy Club Battle uses your microphone only when it is your turn on stage.</p><MicTest state={micState} level={micLevel} test={setupMic}/><div className="permission-list"><span className={micState==="ready"?"ok":""}>MICROPHONE PERMISSION <b>{micState==="ready"?"GRANTED":"REQUIRED"}</b></span><span className={micLevel>.015?"ok":""}>INPUT SIGNAL <b>{micLevel>.015?"DETECTED":"SPEAK TO TEST"}</b></span><span className="ok">AUDIO PLAYBACK <b>READY</b></span></div><GameButton primary disabled={micState!=="ready"||busy} onClick={create}>{busy?"CREATING PROFILE...":"ENTER THE CLUB →"}</GameButton></>}
    {error&&<p className="onboarding-error">{error}</p>}</section></main>;
}

function ClubGame({user}:{user:AccountUser}){
  const identity=useMemo(()=>({id:user.id,username:user.nickname,avatar:user.avatarUrl}),[user]);
  ...
  const pull=useCallback(async()=>{
    if(!lobbyCode)return;
    try{const next=await gameApi.snapshot(lobbyCode);setServerOffset(next.serverNow-Date.now());setSnap(next);setError(null)}catch(e){setError(e instanceof Error?e.message:"SERVER CONNECTION LOST")}
  },[lobbyCode]);

  useEffect(()=>{const timer=setInterval(()=>setClock(Date.now()),100);return()=>clearInterval(timer)},[]);
  useEffect(()=>{const active=sessionStorage.getItem("ccb-active-lobby"),invited=new URLSearchParams(location.search).get("join"),code=active||invited;if(!code)return;const restore=active?gameApi.snapshot(code):gameApi.join(code,identity.id,identity.username);void restore.then(next=>{setServerOffset(next.serverNow-Date.now());setSnap(next);if(invited)history.replaceState({},"",location.pathname)}).catch(e=>{sessionStorage.removeItem("ccb-active-lobby");setError(e instanceof Error?e.message:"LOBBY NOT FOUND")})},[identity.id,identity.username]);
...
function GameHeader({screen,onScreen,onSettings,identity,muted,toggleSound}:{screen:Screen;onScreen:(s:Screen)=>void;onSettings:()=>void;identity:{id:string;username:string};muted:boolean;toggleSound:()=>void}){
  return <header className="game-header"><button className="wordmark focusable" onClick={()=>onScreen("menu")} aria-label="Comedy Club Battle home"><span className="mic-mark">●<i/></span><b>COMEDY<small>CLUB BATTLE</small></b></button>
    ...
    <div className="header-tools"><button className="sound-pill" onClick={toggleSound} aria-label={muted?"Enable sound":"Mute sound"}>{muted?"×":"♪"}</button><button className="player-pill" onClick={()=>onScreen("profile")}><i/><span><b>{identity.username}</b><small>CLUB COMEDIAN</small></span><em>{identity.username[0]}</em></button><button className="icon-button" onClick={onSettings} aria-label="Open settings">⚙</button></div>
  </header>
}
...
function MatchIntro({snap,remaining}:{snap:LobbySnapshot;remaining:number}){
  ...
  return <section className="lineup-screen game-screen center-stage"><p className="kicker">TONIGHT AT COMEDY CLUB BATTLE</p><h1>TONIGHT’S <em>LINEUP</em></h1><div className="lineup-reveal">{snap.lineup.map((id,i)=>{const p=snap.players.find(x=>x.id===id);return <div key={id} style={{"--delay":`${i*.12}s`} as React.CSSProperties}><b>0{i+1}</b><span>{p&&<Avatar value={p.avatar} name={p.username}/>}</span><h3>{p?.username}<small>{rankTitle(p?.rating||1000)}</small></h3>{i===0&&<em>FIRST UP</em>}</div>})}</div><p className="next-state">FIRST UP IN {Math.ceil(remaining/1000)}...</p></section>
}
...
function Avatar({value,name}:{value?:string;name:string}){const [failed,setFailed]=useState(false);return value?.startsWith("/")&&!failed?<img className="real-avatar" src={value} alt={`${name} profile`} onError={()=>setFailed(true)}/>:<span>{name.slice(0,1).toUpperCase()}</span>}
